...
CURL_OPTIONS="--http1.0 --connect-timeout 120 --max-time 120 --fail --silent"
...
#test 5 correct POST and 6 generate cookie
    ...
    echo "\nCookie (sessionID) for user: $SESSION_COOKIE5"
#test 6 POST
    SESSION_COOKIE6=$(curl -i -v -X POST -H "username: Jerry" -H "password: 4W61E0D8P37GLLX" "http://${SERVER_IP}:${SERVER_PORT}/" | grep -i 'Set-Cookie' | cut -d ' ' -f 2 | cut -d '=' -f 2)
    echo "\nCookie (sessionID) for user: $SESSION_COOKIE6"

#test 7  wrong cookie
    output=$(curl $CURL_OPTIONS -v -X GET -H "Cookies: sessionID=ea9394181" "http://${SERVER_IP}:${SERVER_PORT}/file.txt")
    echo "output: $output"
#test 8 right cookie
    output = $(output=$(curl $CURL_OPTIONS -v -X GET -H "Cookies: sessionID=$SESSION_COOKIE5" "http://${SERVER_IP}:${SERVER_PORT}/file.txt"))
